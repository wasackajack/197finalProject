...
// for adding a quiz
//{"title":"samplequiz", "author":"user", "questions":["q1","q2"],"choices":[["a","b","c","d"],["a","b","c","d"]],"answers":["a","b"]}
router.post('/quiz/add', isAuthenticated, async (req, res) => {
  const { title, author, questions, choices, answers} = req.body

  try {
    await Quiz.create({ title, author, questions, choices, answers })
...
router.post('/quiz/addscore', isAuthenticated, async (req, res) => {
  ...
  try {
    await Quiz.findOneAndUpdate({ _id: _id },{ $push: { scores: score }})
